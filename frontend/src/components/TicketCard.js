...
import { FiMessageSquare, FiClock, FiUser, FiArrowRight } from 'react-icons/fi';
        ...
        <div className="flex items-center gap-3">
          {ticket.replies?.length > 0 && (
            <span className="flex items-center gap-1">
              <FiMessageSquare size={11} />{ticket.replies.length}
            </span>
          )}
          <span className="flex items-center gap-1">
            <FiClock size={11} />{formatRelative(ticket.createdAt)}
          </span>
        </div>
      </div>
    </Link>
